...
export default function Signup() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await api.post('/auth/signup', { email, name, password, role }, { withCredentials: true });
          ...
          <form className="w-full flex flex-col" onSubmit={handleSignup}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <button 
                ...
                onClick={async () => {
                  try {
                    const { data } = await api.get(`/auth/socials/oauth2/google/login?role=${role}`);
                    if (data?.authorizationUrl) {
                      window.location.href = data.authorizationUrl;
                      return;
                    }
                    toast.error('Google OAuth link unavailable.');
                  } catch (err: any) {
                    const message = err?.response?.data?.message || 'Failed to start Google OAuth.';
